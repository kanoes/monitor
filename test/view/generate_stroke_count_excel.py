...
class TestGenerateStrokeCountExcel(unittest.TestCase):

    def setUp(self):
        """テスト実行前の初期化処理"""
        days_range = 30

        # Calculate time range in JST, convert to UTC for Azure API
    ...
    def print_the_query_result(self, print_query_type: str):
        self.setUp()
        processed_data = self.analytics_service.fetch_and_process_data(
                self.start_time, 
                self.end_time
            )
        processed_data = processed_data["stroke_count_results"][print_query_type]
        filepath = f"test/view/test_excel/{print_query_type}.xlsx"
        generate_stroke_count_excel(response=processed_data["data"], filepath=filepath)
...
def main():
    ...
    # テストクラスのインスタンスを作成
    test_instance = TestGenerateStrokeCountExcel()
    processed_data = test_instance.print_the_query_result(print_query_type="ca_analysis_stroke_count")
